...
export default function Portfolio() {
  ...
  return (
    <div className={`min-h-screen flex justify-between lg:px-5 lg:flex-col lg:justify-center md:px-5 md:flex-col items-center px-32 sm:flex-col-reverse sm:py-10 overflow-hidden bg-black -z-10 ${isRTL ? 'flex-row-reverse' : 'ltr'}`}>
      <div className={`relative items-start ${isRTL ? 'text-right' : 'text-left'} w-1/3 max-h-64 lg:w-full md:w-full sm:w-full md:justify-center`}>
        <p className="text-lg text-[#F99D1C] font-bold uppercase">{t('portfolio.title')}</p>
        <p className={`text-white text-wrap text-[0.9rem] leading-6 h-60 ${isRTL ? 'text-right' : 'text-left'}`}>
          {t('portfolio.desc')}
        </p>
