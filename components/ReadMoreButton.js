...
const ReadMoreButton = ({ title }) => {
    ...
    const marginRight = locale === 'fa' ? '5px !important' : '0';


    return (
        <div className={`flex items-center space-x-2 w-44 p-1  ${isRTL ? 'justify-start' : ''}`}>
            <button className="capitalize text-white">{title}</button>
            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24" fill="none" className={`flex-shrink-0 ${isRTL ? 'rotate-180' : ''}`} style={{marginRight}}>
